...
public class PoolCue : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            var startLinePos = Camera.main.ScreenToWorldPoint(Input.mousePosition); // Start line wing
            if (ball != null && ball.IsCollidingWith(startLinePos.x, startLinePos.y))
            {
                drawnLine = lineFactory.GetLine(startLinePos, ball.Position.ToUnityVector2(), 1, Color.black);
